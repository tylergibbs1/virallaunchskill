# Output Pack

Use this structure for the final deliverable.

```markdown
# Launch Pack: [Product]

## Executive Summary
[One paragraph: selected angle, audience, bold claim, and why it should work.]

## Assumptions and Placeholders
[List any assumptions, missing proof, unsupported metrics, or placeholders that need human/product-team confirmation.]

## Research Summary
### Sources Reviewed
- [Source](url): why it mattered

### Market Pain
[Top pains in customer language.]

### Category Cliches to Avoid
- ...

### Competitor/Category Opening
[Where the launch can counter-position.]

## Positioning
### Bold Claim
[Selected claim.]

### Why This Claim
[Evidence and reasoning.]

### Rejected Claims
| Claim | Why rejected |
|---|---|

## Hook Iterations
| Hook | Score | Notes |
|---|---:|---|

## Platform Strategy
[Target X/Twitter, LinkedIn, YouTube, or a combination. Explain how the angle changes by platform.]

## Final Launch Drafts
### X/Twitter
[Final post or thread. Omit only if not requested.]

### LinkedIn
[Final LinkedIn launch post. Omit only if not requested.]

### YouTube
[Final YouTube launch video concept. Omit only if not requested.]

## YouTube Launch Video
### Titles
- ...

### Thumbnail Promise
[Visual promise and 3-6 words of thumbnail text.]

### Shorts Script
| Beat | Visual | Voiceover/Text | Purpose |
|---|---|---|---|

### Long-form Demo Outline
| Section | Visual | Voiceover/Text | Retention Purpose |
|---|---|---|---|

### Pinned Comment
[Pinned comment CTA.]

## Demo/Video Script
| Beat | Visual | Voiceover/Text | Purpose |
|---|---|---|---|

## Weapons Check
| Line | Keep/Cut/Rewrite | Novelty 1-10 | Intensity 1-10 | Reason | Revision |
|---|---:|---:|---:|---|---|

## Distribution Plan
- Launch account:
- Founder/employee support:
- Reply strategy:
- Quote-post angles:
- LinkedIn comment/repost strategy:
- LinkedIn founder/company page coordination:
- YouTube title/thumbnail test:
- YouTube pinned comment:
- YouTube-to-X/LinkedIn repurposing:
- Communities and channels:
- Follow-up posts:

## Human Edit Notes
[What a human editor should judge in the final 5%.]

## Risk Notes
[Overclaims, unclear proof, sensitive claims, or claims needing approval.]
```
